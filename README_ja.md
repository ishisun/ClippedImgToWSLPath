...
- **イベント駆動型監視**: AddClipboardFormatListener APIによるクリップボード変更の即時検出
- 重複画像の検出（SHA256ハッシュによる）
- 保存先フォルダのカスタマイズ可能
- 画像保存時に通知を表示
- **デバッグログ機能**: トラブルシューティング用のログ出力オプション
- **自動アイコン生成**: アイコンファイルがない場合は自動生成
- **プロジェクトモード**: プロジェクトディレクトリへの画像保存と相対パスの使用（Claude Code連携用）
...
- **設定ファイル**: `settings.json`（アプリケーション実行フォルダ内）
  - 設定は自動的に保存され、再起動後も維持されます
  - 保存内容: 保存先フォルダ、ログ出力設定、プロジェクトモード設定

### プロジェクトモード

プロジェクトモードは、Claude Codeとのシームレスな連携のために設計されています：

- **有効化**: 設定画面で「Enable Project Mode」をチェック
- **プロジェクトルート**: プロジェクトのルートディレクトリを選択
- **スクリーンショットフォルダ**: サブディレクトリ名を設定（デフォルト: `screenshots`）

有効時の動作:
- 画像は `{プロジェクトルート}/{スクリーンショットフォルダ}/` に保存されます
- クリップボードには相対パスがコピーされます: `screenshots/clipboard_20240125_143052.png`
- Claude Codeでの画像参照に最適です
...
- **クリップボード監視**: AddClipboardFormatListener APIによるイベント駆動型監視
- **画像形式**: PNG形式で保存
